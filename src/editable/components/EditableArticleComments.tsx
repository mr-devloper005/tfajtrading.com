...
export function EditableArticleComments({ slug, comments = [] }: { slug: string; comments?: Comment[] }) {
  ...
  const persist = (next: Comment[]) => {
    ...
    try {
      ...
      // Storage can fail in private contexts; keep the in-memory list.
    }
  }
  ...
  return (
    <section className="mt-14 border-t border-[var(--tk-line)] pt-10">
      <div className="flex items-center gap-2 text-lg font-bold text-[var(--tk-text)]">
        <MessageCircle className="h-5 w-5 text-[var(--tk-accent)]" /> Comments
        <span className="text-[var(--tk-muted)]">({all.length})</span>
      </div>

      <form onSubmit={submit} className="mt-6 rounded-[1.8rem] border border-[var(--tk-line)] bg-[var(--tk-surface)] p-5 shadow-[0_18px_42px_rgba(34,26,15,0.06)]">
        <input
          value={name}
          onChange={(event) => setName(event.target.value)}
          placeholder="Your name (optional)"
          maxLength={60}
          className="h-12 w-full rounded-[1rem] border border-[var(--tk-line)] bg-[var(--tk-bg)] px-4 text-sm text-[var(--tk-text)] outline-none transition focus:border-[var(--tk-accent)]"
        />
        <textarea
          value={text}
          ...
          rows={4}
          maxLength={1500}
          className="mt-3 w-full resize-y rounded-[1rem] border border-[var(--tk-line)] bg-[var(--tk-bg)] px-4 py-3 text-sm leading-6 text-[var(--tk-text)] outline-none transition focus:border-[var(--tk-accent)]"
        />
      ...
      <div className="mt-6 grid gap-4">
        {all.map((comment) => (
          <div key={comment.id} className="rounded-[1.6rem] border border-[var(--tk-line)] bg-[var(--tk-surface)] p-5 shadow-[0_14px_36px_rgba(34,26,15,0.05)]">
